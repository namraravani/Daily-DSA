...
int main()
{
    ...
    for (auto &[key, value] : parsed.items())
    {
        if (key != "keys")
        {
            int x = stoi(key);                            // Convert key to integer (x)
            int base = stoi(value["base"].get<string>()); // Convert base to integer
            string y = value["value"].get<string>();      // Get value as string
            equations[x] = {base, y};                     // Store as pair of base and value
        }
    }
